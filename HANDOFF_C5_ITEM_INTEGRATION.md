...
| `misc` | その他 | 2 |
| `mega_stone` | メガストーン (個別42件 = 詳細あり23 + skeleton 18 + 旧マーカー1) | 42 |
...
1. **メガストーン個別化 (2026-05-16 完了)** — 41種を追加 (詳細あり23 + skeleton 18)。スキーマ拡張: `mega_form` / `mega_types` / `mega_ability` / `mega_ability_desc` / `mega_stats` / `acquisition` / `applies_to`
2. `verify: true` のゲーム内確認 (現在 23 件 — 内訳: 既存 5 件 + メガ skeleton 18 件)
3. **メガストーン残り約20種の調査** (本家のメガミュウツーX/Y、メガラティオス、メガラティアス、メガフーディン、メガジュペッタ、メガサメハダー、メガバンギラス、メガクチート、メガデンリュウ、メガミミロップ、メガプテラ等がポケチャンに実装されているか不明)
4. **メガ skeleton 18 種の詳細追加** (チュートリアル8 + フロンティアショップ旧来10、メガ後タイプ・特性・種族値が未収集)
...
着手時:
- [ ] `_review/items_database.json` を読み、件数 (114) とスキーマを確認
- [ ] 「実装済み 100 / 未実装 14 / メガストーン 42 (詳細23 + skeleton18 + マーカー1)」の内訳を理解
