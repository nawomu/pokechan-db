/* ===================================================================
 * waza_picker_v3.js — 技選択 UI 共通ロジック(pokedb.js 1本読み版・②W19)
 *
 * ★何を変えたか(waza_picker.jsからの差分。ロジック・見た目は無改変。データの取り方だけ変えた):
 ...
 *   (waza-list_v3.html='champions'固定・waza-list_all_v3.html='all'固定。URLの?data=切替は無い)。
 *
...
function _setupNewTagFilter() {
  ...
  function detailCategory(t) {
    ...
    if (/^☀️ 天候|^🌤 天候|^🌐 フィールド変更|^🌿 フィールド|^🌀 ルーム|^🌬|^🌌 重力|フィールドで威力|追い風|^🪞 直前/.test(t)) return 'field';
    // 設置/守る(壁・設置・ワイガファガ・まもる貫通)
    if (/^📌 設置|^🛡️ 壁|^🛡 壁|^🚧|^🛡️ まもる|まもる貫通|ステルスロック|^🕸|^🛡 ワイドガード|^🛡 ファストガード/.test(t)) return 'hazard';
    // 解除系(フィールド破壊・壁破壊・設置解除・防御貫通・ランクリセット)
    if (/^🧹|^🌐 フィールド破壊|壁破壊|フィールド破壊|防御貫通|ランクリセット/.test(t)) return 'clear';
    // 解除系
    ...
    if (/^🔗 バインド|^🔄 相手交代|^↩️ 自分交代|^🪤 交代不可/.test(t)) return 'switch';
    // 場の効果(復活した追い風・重力)
    if (/^🌬️ 追い風|^🌌 重力/.test(t)) return 'field';
    // 設置/守る(復活したまもる貫通)
    if (/^🛡️ まもる貫通/.test(t)) return 'hazard';
    // 解除系(復活した設置解除)
    if (/^🧹 設置解除/.test(t)) return 'clear';
    // ダメ補正(復活した失敗ダメージ)
    if (/^💔 失敗ダメージ/.test(t)) return 'dmg';
    return 'misc';
  }
  const CAT_LABEL = {
