...
> ポケチャン実装済の **きのみ系 (10件) + HPドレイン系 (2件)** は「ダメージ受けた後 / ターン終了時」に発動するため、現状の damage 一発計算型シミュレータには直接組み込めない。
...
| key | name | 発動条件 |
|---|---|---|
| `kaifuku_no_kona` (仮) | かいふくのこな / ? | ターン終了時 +HP |
| `tabesashino_iyashi` (仮) | ? | 同上 |

→ 具体的な key は `items_database.js` 確認要。
