...
| 3 | 中国語 Psychic の扱い | **`超能力`** (3 文字、修正済 — 旧 `超能` 2 文字から変更) |
| 4 | 日本語 2 文字タイプ | **2 文字のまま** (`みず` / `くさ` 等) |

→ `types-master.json` は中国語 Psychic のみ修正(`超能` → `超能力`)。他 3 件は採用案を確定値として継続使用。
→ 修正後の 9 言語 × 18 タイプ unique 性は再検証で OK 確認済 (`HANDOFF_DB02_TO_DB01_2026_05_18_TASK_VA_D.md` 参照)。

---

### 元の判断仰ぎ案(以下は履歴。確定値は上記表)
