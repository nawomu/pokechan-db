...
function freshSide(pokeName, moveKeys, opts) {
  ...
  s.moves = keys.map(k => data.WAZA_MAP[k]);
  s.selectedMoveIdx = 0;
...
function benchEntry16(pokeName, moveKey, opts){
  ...
  return { poke: pokeByName(pokeName), effort: {hp:0,atk:0,def:0,spatk:0,spdef:0,spd:0},
    ...
    moves: moveKey ? [data.WAZA_MAP[moveKey]] : [], currentHp: null, fainted: false, status: 'none', sleepTurns: null };
}
...
// ─────────────────────────────────────────────
console.log('\n' + '='.repeat(60));
console.log(`難所相互作用16件テスト結果: pass=${pass} fail=${fail} skip=${skip}`);
if (fails.length) {
  console.log('\n--- FAIL一覧 ---');
