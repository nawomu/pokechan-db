...
// ★P8: 世代チップ(確認ビュー用・軽量)
function genChipConfirm(av) {
  if (!av) return '<span style="color:#888">—</span>';
  const g = av.gen_introduced || '?';
  if (av.is_lgpe) return `<b style="color:#c0a0ff">G${g}L</b>`;
  const gens = av.gens;
  if (!gens) return `<span style="color:#aaa">G${g}</span>`;
  const last = gens[gens.length - 1];
  if (last < 9) return `<span style="color:#ff8080">G${g}-${last}</span>`;
  return `<span style="color:#80c0ff">G${g}~</span>`;
}
...
function buildRow(m) {
  ...
  const status = m._national
    ? `<span class="mv-st newn">🆕新規</span>`
    : holes.length === 0
  ...
  <td class="col-season">${m._national ? '<span class="season-bdg s-new" title="全国版で新規追加(M-A/M-B以外)">🆕新規</span>' : (MB_NEW_TECH_NAMES.has(m.name) ? '<span class="season-bdg s-mb" title="レギュMBで新規追加">🆕M-B</span>' : '<span class="season-bdg s-ma" title="M-A(もとから・既存)">M-A</span>')}</td>
  ${prioTd}
  ...
  <td class="col-gen-c">${genChipConfirm(m.availability)}</td>
</tr>`;
}
...
const nationalMoves = Object.values(_ALL.WAZA_MAP).filter(w => w.national_new).map(w => Object.assign({}, w, {
  _national: true,
  effect_en: _EFFEN[w.key] || '',
  battle_data: Object.assign({}, w.battle_data || {}, { priority: w.priority || 0 }),
}));
const moves = [...Object.values(map), ...nationalMoves];
...
for (const m of moves) {
  ...
  // 全国版の純ダメージ技(効果なし)はNOEFFに入れず専用セクションのみに(NOEFF肥大化防止)。効果ありなら各カテゴリに混ぜる。
  const buckets = kinds.length ? kinds : (m._national ? [] : [NOEFF]);
  for (const k of buckets) { if (!byKind.has(k)) byKind.set(k, []); byKind.get(k).push(m); }
}
...
// ★2026-07-01 全国版新技は今や全effectsを持つので、効果カテゴリー別に分散済(🆕新規バッジで区別)。
//   一塊の専用セクションは廃止し、effectsを持たない純ダメージ技(=どのkind枠にも入らない分)だけを小バケツにまとめる。
const NATIONAL_NEW_KIND = '🆕 新技・純ダメージ(全国版・効果なし)';
const nationalNew = moves.filter(m => m._national && !(((m.battle_data || {}).effects || []).length));
if (nationalNew.length) byKind.set(NATIONAL_NEW_KIND, nationalNew);

const ordered = [NEW_TECHS_KIND, ...KIND_ORDER.filter(k => byKind.has(k)), ...restKinds, ...(byKind.has(NATIONAL_NEW_KIND) ? [NATIONAL_NEW_KIND] : []), ...(byKind.has(NOEFF) ? [NOEFF] : [])].filter(k => byKind.has(k));
...
// ★2026-07-01 全国版427技を全数 web照合＋ヤック意味照合で作り直したため、過去の確定/作業中はリセット。
//   全effect-kind=「確認待ち(ビルド済・阿部さんの耳まち)」からやり直す。阿部さんが耳でOKしたkindをここに足していく。
const CONFIRMED = new Set([]); // ✓確定(阿部さんの耳でOK済) ← 阿部さんだけが足す
const WORKING = new Set([]);   // 🔨 いま開通作業中(任意・複数可)
...
const _txt = m => { if (!_composeCache.has(m)) _composeCache.set(m, (m._national ? (m.description || '') : (compose(m).text || ''))); return _composeCache.get(m); };
...
.mv-st.newn { background:#E8F5E9; color:#1B5E20; border:1px solid #81C784; }
...
.season-bdg.s-new { background:#2E7D32; color:#fff; }
