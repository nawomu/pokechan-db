...
// ★全国版で新規追加した技(M-A/M-B以外)の専用セクション(全445をまとめて確認)
const NATIONAL_NEW_KIND = '🆕 新技(全国版追加・M-A/M-B以外)';
const nationalNew = moves.filter(m => m._national);
if (nationalNew.length) byKind.set(NATIONAL_NEW_KIND, nationalNew);

const ordered = [NEW_TECHS_KIND, NATIONAL_NEW_KIND, ...KIND_ORDER.filter(k => byKind.has(k)), ...restKinds, ...(byKind.has(NOEFF) ? [NOEFF] : [])].filter(k => byKind.has(k));
