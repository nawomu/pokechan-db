...
const sections=GROUPS.map(([g,label,color])=>{const rs=rows.filter(r=>group(r)===g);if(!rs.length)return '';
  ...
  <table><thead><tr><th>✓</th><th>No</th><th>技名</th><th>en</th><th>型</th><th>分類</th><th>威</th><th>命</th><th>PP</th><th>優</th><th>状態</th><th>説明文(JA)</th><th>公式effect(en)</th></tr></thead><tbody>${rs.map(rowHtml).join('')}</tbody></table></section>`;}).join('');
...
function cands(v){const sj=v.species_names.ja||'';const c=[sj];if(v.is_mega){let s='';if(/-x$/.test(v.form_slug))s='X';else if(/-y$/.test(v.form_slug))s='Y';c.push('メガ'+sj+s);}for(const k in FORM_SUF)if((v.form_slug||'').includes(k))c.push(sj+FORM_SUF[k]);return c;}
...
<td>${r.inDB?'<span class="b ok">DB有</span>':(r.is_default?'<span class="b miss">未収録</span>':'<span class="b form">フォルム</span>')}</td><td class=ja>${esc(r.projName)}</td></tr>`).join('');
const html=`<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
...
<div class="prog">📊 <b>DB収録(推定) ${inDB}</b> / 全variety ${rows.length} ｜ 確認済 <span id="ck">0</span>
...
<table><thead><tr><th>✓</th><th>図鑑</th><th>名前</th><th>en</th><th>タイプ</th><th>H/A/B/C/D/S</th><th>合計</th><th>特性</th><th>状態</th><th>DB名</th></tr></thead><tbody>${tr}</tbody></table>
