...
function clause(e, m) {
  ...
  switch (k) {
    ...
    case 'じゅうりょく':
      return `${durT(e.duration)}の間、場の全員の命中率が5/3倍くらいになり、ひこうタイプや特性「ふゆう」で浮いているポケモンも地面にいるあつかいになる。「そらをとぶ」「とびはねる」「フリーフォール」などの空中に行く技は使えなくなり、使っている最中なら中止される`; // 2026-06-17: 5/3倍と中止仕様を明示
    case '必ず急所':
      return `必ず急所に当たる`;
    case 'ランク無視':
    ...
    case 'みがわり設置':
      ...
      // ★2026-06-18: ダイマックス相互作用(防げるが、自分がダイマックスすると消える)を末尾に(SYSTEMS_IN_GAME.dynamax=false時はカッコ書き)
