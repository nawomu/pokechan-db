...
// 全部版(pokechan_data_all.js)は技キーがPokeAPIスラッグ・独自メガ等でContent自体が存在しないことがある。
...
} catch (__e) { skipCase('セクション1: こだわりハチマキ/メガネ × ダメージ倍率', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション2: こだわりロック(技固定)', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション3: きあいのタスキ(HP1耐え)', (__e && __e.message) || String(__e)); }
...
try {
  ...
  let sturdySeed = -1;
  const kai = pokeByName('カイリュー');
  const fore = pokeByName('フォレトス');
...
} catch (__e) { skipCase('セクション4: がんじょう(HP1耐え・消費なし)', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション5: マルチスケイル(満タン半減) × かたやぶり貫通', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション6: ふゆう(じめん技無効) × かたやぶり貫通', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション7: いのちのたま(ダメージ増加・反動)', (__e && __e.message) || String(__e)); }
...
try {
  ...
  resetEnv();
} catch (__e) { skipCase('セクション8: マジックガード × 砂嵐ダメージ無効', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション9: ちからもち/ヨガパワー × ダメージ倍率', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション10: たべのこし(ターン終了回復)', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション11: オボンのみ(HP≤50%で発動・1回限り)', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション12: トレース(相手の特性コピー)', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('セクション13: かわりもの(へんしん) × 持ち物非コピー', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('未実装/部分実装 特性・持ち物の確認', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('block@line804', (__e && __e.message) || String(__e)); }
...
try {
  ...
  const fgMul = fg && fg.battle_data && fg.battle_data.effects && fg.battle_data.effects[0]
    && fg.battle_data.effects[0].consecutive_success_multiplier;
  if (fgMul === undefined) {
    // 全部版のbattle_dataにはconsecutive_success_multiplier(Championsのバランス調整専用フィールド)が無い=content差分でなくdata差分
    skipCase('S16-T8 ファストガード: 連続使用ペナルティのデータ宣言', 'battle_data.effects[0].consecutive_success_multiplierが無い(全部版データに未収録)');
  } else {
    check('S16-T8 ファストガード: 連続使用ペナルティのデータ宣言',
      fgMul === 0.3333,
      JSON.stringify(fg && fg.battle_data.effects[0]).slice(0,120));
  }
} catch (__e) { skipCase('block@line845', (__e && __e.message) || String(__e)); }
...
} catch (__e) { skipCase('block@line927', (__e && __e.message) || String(__e)); }

// ─────────────────────────────────────────────
// 結果集計
// ─────────────────────────────────────────────
console.log('\n=== 結果 ===');
console.log(`合計: ${pass + fail}件 / ✅ pass: ${pass} / ❌ fail: ${fail} / ⚪ skip: ${skip}`);
if (fails.length > 0) {
  console.log('FAILリスト:');
  fails.forEach(f => console.log('  ❌ ' + f));
}
if (skips.length > 0) {
  console.log('SKIPリスト(全部版に存在しないコンテンツ等):');
