...
}
{
  ...
  check('T210 せんせいのつめが発動して遅いカビゴンが先攻',
    // 2026-07-18: 自分側の先攻行は「先攻: じぶんの ◯◯」に変更(ミラーマッチ判別・阿部さんFB)
    E.battleLog.slice(t210log).some(l => l.msg.includes('先攻: じぶんの カビゴン')),
...
// 出典: ポケモンWiki「がんじょうあご」「きれあじ」(対象技表)+対象24技を1技ずつsonnetワークフローで個別裏取り
// (wf_3632b4ff・24/24確認)。flags.bite/slash は tools/_fix_add_bite_slash_flags.js で追加。
{
  resetEnv();
  // T286 がんじょうあご: かみつき技1.5倍・対象外は変わらず
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const b0 = E.calcDamage('self', 'opp', moveByName('かみくだく')).max;
  const q0 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  E.sides.self.ability = 'がんじょうあご';
  const b1 = E.calcDamage('self', 'opp', moveByName('かみくだく')).max;
  const q1 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  check('T286 がんじょうあごでかみつき技1.5倍(対象外は不変)', b1 > b0 && q1 === q0,
    `bite:${b0}→${b1} other:${q0}→${q1}`);
  // T286b きれあじ: 切る技1.5倍・対象外は変わらず
  E.sides.self = freshSide('フシギバナ', null);
  const s0 = E.calcDamage('self', 'opp', moveByName('リーフブレード')).max;
  E.sides.self.ability = 'きれあじ';
  const s1 = E.calcDamage('self', 'opp', moveByName('リーフブレード')).max;
  const q2 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  check('T286b きれあじで切る技1.5倍(対象外は不変)', s1 > s0 && q2 === q0,
    `slash:${s0}→${s1} other:${q2}`);
  resetEnv();
}
...
console.log('\n=== 段135 こだわりロック(こだわりスカーフ=選択技固定) ===');
// 出典: items_database宣言(restriction:選択技固定)+ポケモンWiki「こだわりスカーフ」。
// 持っている間、最初に使った技しか出せない。交代・道具を失うと解除。
{
  resetEnv();
  // T288 こだわりスカーフ: 一度使った技に固定される(別の技を選んでも最初の技が出る)
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.self.moves = [moveByName('でんこうせっか'), moveByName('ギガドレイン')];
  E.sides.self.selectedMoveIdx = 0;
  E.sides.self.item = 'kodawari_scarf';
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const oMax = E.realStat(E.sides.opp, 'hp');
  E.sides.opp.currentHp = oMax;
  E.setRandom(() => 0.0);
  E.runSingleAttack('self', 0);   // でんこうせっかを使用→ロック
  const afterFirst = E.sides.opp.currentHp;
  const qDmg = oMax - afterFirst;
  E.runSingleAttack('self', 1);   // ギガドレインを選んでも…
  const second = afterFirst - E.sides.opp.currentHp;
  check('T288 こだわり中は最初の技しか出ない(2回目も同ダメージ=でんこうせっか)',
    second === qDmg, `1回目=${qDmg} 2回目=${second}`);
  // T288b 交代でロック解除(戻して別の技が出せる)
  const subK = freshSide('ピカチュウ', null);
  subK.moves = [moveByName('10まんボルト')];
  E.sides.self.bench = [{ poke: subK.poke, effort: subK.effort, natureIdx: 0, ability: '', item: '',
    moves: subK.moves, currentHp: null, fainted: false, status: 'none', sleepTurns: null, lastConsumedItem: null, megaBase: null }];
  E.attemptSwitch('self', 0);   // 引っ込める(ロック解除)
  E.attemptSwitch('self', 0);   // フシギバナを戻す
  check('T288b 交代でロックが解除される', !E.sides.self.choiceLock,
    `lock=${E.sides.self.choiceLock && E.sides.self.choiceLock.name}`);
  // T288c AIもロック中はその技を選ぶ
  E.sides.self.item = 'kodawari_scarf';
  E.sides.self.selectedMoveIdx = 1;
  E.runSingleAttack('self', 1);   // ギガドレインでロック
  check('T288c AIはロックされた技を選ぶ', E.aiChooseMove('self') === 1,
    `idx=${E.aiChooseMove('self')} lock=${E.sides.self.choiceLock && E.sides.self.choiceLock.name}`);
  resetEnv();
}

console.log('\n=== 段136-137 実戦級特性10種 ===');
// 出典: ABILITY_DESC(公式準拠)。はやてのつばさ(HP満タン時のみ=第7世代仕様がABILITY_DESCに織り込み済み)/
// おやこあい(25%の2発目)/かそく/ポイズンヒール/スナイパー(2.25倍)/スキルリンク(常に最大回数)/
// せいぎのこころ/ふくつのこころ/アナライズ/かんそうはだ。
{
  resetEnv();
  // T289 はやてのつばさ: HP満タンならひこう技が先制(遅いカビゴンが先に動ける)
  E.sides.self = freshSide('カビゴン', null);
  E.sides.self.moves = [moveByName('つばめがえし')];
  E.sides.self.selectedMoveIdx = 0;
  E.sides.self.ability = 'はやてのつばさ';
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.sides.opp.selectedMoveIdx = 0;
  E.setRandom(() => 0.99);
  check('T289 HP満タンでひこう技が先制', E.decideOrder(E.sides.self.moves[0], E.sides.opp.moves[0]) === 'self',
    `first=${E.decideOrder(E.sides.self.moves[0], E.sides.opp.moves[0])}`);
  E.sides.self.currentHp = 10;
  check('T289b HPが減っていると先制しない', E.decideOrder(E.sides.self.moves[0], E.sides.opp.moves[0]) === 'opp',
    `first=${E.decideOrder(E.sides.self.moves[0], E.sides.opp.moves[0])}`);
  resetEnv();
}
{
  resetEnv();
  // T289c おやこあい: 2回攻撃(合計 > 1発分・2発目=25%)
  E.sides.self = freshSide('ガルーラ', null);
  E.sides.self.moves = [moveByName('でんこうせっか')];
  E.sides.self.selectedMoveIdx = 0;
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const oMax = E.realStat(E.sides.opp, 'hp');
  E.sides.opp.currentHp = oMax;
  E.setRandom(() => 0.0);
  const d1 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).min;
  const dq = E.calcDamage('self', 'opp', moveByName('でんこうせっか'), {powerOverride: Math.max(1, Math.floor(moveByName('でんこうせっか').power * 0.25))}).min;
  E.sides.self.ability = 'おやこあい';
  E.phaseDealDamage('self', 'opp', moveByName('でんこうせっか'));
  check('T289c おやこあいで2発(本気+25%)', oMax - E.sides.opp.currentHp === d1 + dq,
    `dealt=${oMax - E.sides.opp.currentHp} 期待=${d1}+${dq}`);
  resetEnv();
}
{
  resetEnv();
  // T289d かそく: ターン終了ですばやさ+1
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.self.ability = 'かそく';
  E.sides.self.moves = [moveByName('つるぎのまい')];
  E.sides.self.selectedMoveIdx = 0;
  E.sides.opp = freshSide('カビゴン', 'hataku');
  E.sides.opp.status = 'paralysis';
  E.setRandom(() => 0.5);
  E.runTurn();
  check('T289d かそくで毎ターンすばやさ+1', E.sides.self.rank.spd === 1, `spd=${E.sides.self.rank.spd}`);
  resetEnv();
}
{
  resetEnv();
  // T289e ポイズンヒール: どくでダメージを受けず1/8回復
  E.sides.self = freshSide('グライオン', null);
  E.sides.self.ability = 'ポイズンヒール';
  E.sides.self.status = 'badpoison';
  const gMax = E.realStat(E.sides.self, 'hp');
  E.sides.self.currentHp = gMax - 40;
  E.phaseSlipFor('self');
  check('T289e ポイズンヒールでどくが回復に変わる', E.sides.self.currentHp === gMax - 40 + Math.floor(gMax / 8),
    `hp=${E.sides.self.currentHp} 期待=${gMax - 40 + Math.floor(gMax / 8)}`);
  resetEnv();
}
{
  resetEnv();
  // T289f スナイパー: 急所が1.5→2.25倍
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.self.critical = true;
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const c0 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  E.sides.self.ability = 'スナイパー';
  const c1 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  check('T289f スナイパーで急所ダメージが伸びる(2.25/1.5)', c1 > c0, `${c0}→${c1}`);
  resetEnv();
}
{
  resetEnv();
  // T289g スキルリンク: 2-5回技が必ず5回(最小乱数なら2回のはず)
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.self.ability = 'スキルリンク';
  E.sides.self.moves = [moveByName('タネマシンガン')];
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const oMax2 = E.realStat(E.sides.opp, 'hp');
  E.sides.opp.currentHp = oMax2;
  E.setRandom(() => 0.0);   // 通常なら2回の乱数
  const one = E.calcDamage('self', 'opp', moveByName('タネマシンガン')).min;
  E.phaseDealDamage('self', 'opp', moveByName('タネマシンガン'));
  check('T289g スキルリンクで必ず5回当たる', oMax2 - E.sides.opp.currentHp === one * 5,
    `dealt=${oMax2 - E.sides.opp.currentHp} 期待=${one * 5}`);
  resetEnv();
}
{
  resetEnv();
  // T289h せいぎのこころ: あく技を受けるとこうげき+1
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.self.moves = [moveByName('かみくだく')];
  E.sides.self.selectedMoveIdx = 0;
  E.sides.opp = freshSide('エルレイド', 'hataku');
  E.sides.opp.ability = 'せいぎのこころ';
  E.setRandom(() => 0.5);
  E.runSingleAttack('self', 0);
  check('T289h あく技被弾でこうげき+1', E.sides.opp.rank.atk === 1, `atk=${E.sides.opp.rank.atk}`);
  resetEnv();
}
{
  resetEnv();
  // T289i アナライズ: 相手が行動済みだと威力1.3倍
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.self.ability = 'アナライズ';
  E.sides.opp = freshSide('カビゴン', 'hataku');
  E.sides.opp.movedThisTurn = false;
  const a0 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  E.sides.opp.movedThisTurn = true;
  const a1 = E.calcDamage('self', 'opp', moveByName('でんこうせっか')).max;
  check('T289i アナライズは後攻で1.3倍', a1 > a0, `${a0}→${a1}`);
  resetEnv();
}
{
  resetEnv();
  // T289j かんそうはだ: みず無効+1/4回復・ほのお被弾1.25倍
  E.sides.self = freshSide('フシギバナ', null);
  E.sides.opp = freshSide('カビゴン', 'hataku');
  E.sides.opp.ability = 'かんそうはだ';
  const w = E.calcDamage('self', 'opp', moveByName('ハイドロポンプ'));
  check('T289j かんそうはだでみず技がこうかなし', w && w.immune, `immune=${w && w.immune}`);
  const f0base = E.calcDamage('self', 'opp', moveByName('かえんほうしゃ') || moveByName('だいもんじ'));
  E.sides.opp.ability = '';
  const f1base = E.calcDamage('self', 'opp', moveByName('かえんほうしゃ') || moveByName('だいもんじ'));
  check('T289k かんそうはだはほのお被弾1.25倍', f0base.max > f1base.max, `乾燥肌=${f0base.max} 通常=${f1base.max}`);
  resetEnv();
}
...
console.log('\n=== 段144 レギュMB新持ち物の効果検証(2026-06-19・ひかりのねんど/各種いわ/こうかくレンズ) ===');
{
  // T299 あついいわ持ち: にほんばれ → weatherTurns=8
  resetEnv();
  E.sides.self = freshSide('フシギバナ', 'nihonbare');
  E.sides.opp = freshSide('カビゴン', 'hataku');
  E.sides.self.item = 'atsui_iwa';
  E.setRandom(() => 0.5);
  E.phaseApplyEffects('self', 'opp', moveByName('にほんばれ'));
  check('T299 あついいわ: にほんばれの効果が8ターンに延長', E.env.weatherTurns === 8, `weatherTurns=${E.env.weatherTurns}`);

  // T300 対照: 持ち物なし → weatherTurns=5
  resetEnv();
  E.sides.self = freshSide('フシギバナ', 'nihonbare');
  E.sides.opp = freshSide('カビゴン', 'hataku');
  E.sides.self.item = '';
  E.phaseApplyEffects('self', 'opp', moveByName('にほんばれ'));
  check('T300 対照: 持ち物無しなら5ターン', E.env.weatherTurns === 5, `weatherTurns=${E.env.weatherTurns}`);

  // T301 しめったいわ持ち: あまごい → 8ターン
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'amagoi');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.sides.self.item = 'shimetta_iwa';
  E.phaseApplyEffects('self', 'opp', moveByName('あまごい'));
  check('T301 しめったいわ: あまごい8T延長', E.env.weatherTurns === 8, `weatherTurns=${E.env.weatherTurns}`);

  // T302 さらさらいわ持ち: すなあらし → 8T
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'sunaarashi');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.sides.self.item = 'sarasara_iwa';
  E.phaseApplyEffects('self', 'opp', moveByName('すなあらし'));
  check('T302 さらさらいわ: すなあらし8T延長', E.env.weatherTurns === 8, `weatherTurns=${E.env.weatherTurns}`);

  // T303 つめたいいわ持ち: ゆきげしき → 8T
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'yukigeshiki');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.sides.self.item = 'tsumetai_iwa';
  E.phaseApplyEffects('self', 'opp', moveByName('ゆきげしき'));
  check('T303 つめたいいわ: ゆきげしき8T延長', E.env.weatherTurns === 8, `weatherTurns=${E.env.weatherTurns}`);

  // T304 ひかりのねんど持ち: ひかりのかべ → screenTurns=8
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hikarinokabe');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.sides.self.item = 'hikari_no_nendo';
  E.phaseApplyEffects('self', 'opp', moveByName('ひかりのかべ'));
  const lsTurns = E.sides.self.screenTurns && E.sides.self.screenTurns.lightScreen;
  check('T304 ひかりのねんど: ひかりのかべ8T延長', lsTurns === 8, `screenTurns=${lsTurns}`);

  // T305 対照: ひかりのねんど無し → 5T
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hikarinokabe');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.sides.self.item = '';
  E.phaseApplyEffects('self', 'opp', moveByName('ひかりのかべ'));
  const lsTurns2 = E.sides.self.screenTurns && E.sides.self.screenTurns.lightScreen;
  check('T305 対照: ひかりのねんど無しなら5T', lsTurns2 === 5, `screenTurns=${lsTurns2}`);

  // T306 おおきなねっこ: HP吸収量1.3倍
  resetEnv();
  const drainMove = Object.values(data.WAZA_MAP).find(m => m.name === 'ドレインパンチ' || m.name === 'ギガドレイン');
  if (drainMove) {
    E.sides.self = freshSide('カビゴン', null);
    E.sides.self.moves = [drainMove];
    E.sides.self.item = 'ooki_na_nekko';
    E.sides.opp = freshSide('フシギバナ', 'hataku');
    const maxHp = E.realStat(E.sides.self, 'hp');
    E.sides.self.currentHp = Math.floor(maxHp / 2); // HP半減状態から
    const before = E.sides.self.currentHp;
    E.setRandom(() => 0.5);
    E.phaseDealDamage('self', 'opp', drainMove);
    const heal = E.sides.self.currentHp - before;
    // 持ち物なし対照
    resetEnv();
    E.sides.self = freshSide('カビゴン', null);
    E.sides.self.moves = [drainMove];
    E.sides.self.item = '';
    E.sides.opp = freshSide('フシギバナ', 'hataku');
    E.sides.self.currentHp = Math.floor(maxHp / 2);
    const before2 = E.sides.self.currentHp;
    E.setRandom(() => 0.5);
    E.phaseDealDamage('self', 'opp', drainMove);
    const heal2 = E.sides.self.currentHp - before2;
    check('T306 おおきなねっこ: HP吸収量が1.3倍に増える(または同等以上)', heal >= heal2, `with=${heal}, without=${heal2}`);
  }

  // T307 くろいてっきゅう: 素早さ半分
  resetEnv();
  E.sides.self = freshSide('ピカチュウ', null);
  E.sides.self.item = 'kuroi_tekkyu';
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const spdWith = E.effectiveSpeed(E.sides.self);
  resetEnv();
  E.sides.self = freshSide('ピカチュウ', null);
  E.sides.self.item = '';
  E.sides.opp = freshSide('カビゴン', 'hataku');
  const spdWithout = E.effectiveSpeed(E.sides.self);
  check('T307 くろいてっきゅう: 素早さが半分になる', spdWith === Math.floor(spdWithout / 2), `with=${spdWith}, without=${spdWithout}`);

  // T308 メトロノーム: 同技連続で威力増加
  resetEnv();
  const hataku = moveByName('はたく');
  E.sides.self = freshSide('カビゴン', null);
  E.sides.self.moves = [hataku];
  E.sides.self.item = 'metronome';
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.setRandom(() => 0.5);
  // 1回目(boost=0)
  const r1 = E.calcDamage('self', 'opp', hataku);
  // 2回目(同技 boost=+20%)
  E.sides.self._metronomeLastMoveKey = hataku.key;
  E.sides.self.metronomeCount = 1;
  const r2 = E.calcDamage('self', 'opp', hataku);
  // 5回目(boost=+100% = ×2)
  E.sides.self.metronomeCount = 5;
  const r5 = E.calcDamage('self', 'opp', hataku);
  check('T308 メトロノーム: 連続使用で威力が増える(2回目>1回目)', r2.min > r1.min, `r1=${r1.min}, r2=${r2.min}`);
  check('T308b メトロノーム: 5回目(+100%)が1回目の約2倍', r5.min >= r1.min * 1.8, `r1=${r1.min}, r5=${r5.min}`);

  // ===== T309-T314: レギュMB追加4アイテム(2026-07-11 ヤックン確認・エンジン実装は既存=ここで初検証) =====
  // T309 いのちのたま: ダメージ約1.3倍
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hataku');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.setRandom(() => 0.5);
  E.sides.self.item = '';
  const t309a = E.calcDamage('self', 'opp', moveByName('はたく'));
  E.sides.self.item = 'life_orb';
  const t309b = E.calcDamage('self', 'opp', moveByName('はたく'));
  check('T309 いのちのたま: ダメージ約1.3倍', t309b.min > t309a.min * 1.2 && t309b.min <= Math.ceil(t309a.min * 1.35), `無=${t309a.min}, 珠=${t309b.min}`);

  // T310 いのちのたま: マジックルーム中は無効
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hataku');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.setRandom(() => 0.5);
  E.sides.self.item = 'life_orb';
  E.env.magicRoom = 2;
  const t310 = E.calcDamage('self', 'opp', moveByName('はたく'));
  check('T310 いのちのたま: マジックルーム中は等倍', t310.min === t309a.min, `room=${t310.min}, 素=${t309a.min}`);
  E.env.magicRoom = 0;

  // T311 たつじんのおび: 効果ばつぐん時のみ1.2倍(等倍は無効)
  resetEnv();
  E.sides.self = freshSide('カメックス', 'naminori');
  E.sides.opp = freshSide('リザードン', 'hataku');   // みず→ほのお/ひこう=2倍
  E.setRandom(() => 0.5);
  E.sides.self.item = '';
  const t311a = E.calcDamage('self', 'opp', moveByName('なみのり'));
  E.sides.self.item = 'expert_belt';
  const t311b = E.calcDamage('self', 'opp', moveByName('なみのり'));
  check('T311 たつじんのおび: ばつぐん時 約1.2倍', t311b.min > t311a.min * 1.1 && t311b.min <= Math.ceil(t311a.min * 1.25), `無=${t311a.min}, 帯=${t311b.min}`);
  resetEnv();
  E.sides.self = freshSide('カメックス', 'hataku');
  E.sides.opp = freshSide('カビゴン', 'hataku');   // ノーマル等倍
  E.setRandom(() => 0.5);
  E.sides.self.item = '';
  const t311c = E.calcDamage('self', 'opp', moveByName('はたく'));
  E.sides.self.item = 'expert_belt';
  const t311d = E.calcDamage('self', 'opp', moveByName('はたく'));
  check('T311b たつじんのおび: 等倍では無効', t311d.min === t311c.min, `無=${t311c.min}, 帯=${t311d.min}`);

  // T312 ちからのハチマキ: 物理のみ1.1倍
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hataku');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.setRandom(() => 0.5);
  E.sides.self.item = 'muscle_band';
  const t312 = E.calcDamage('self', 'opp', moveByName('はたく'));
  check('T312 ちからのハチマキ: 物理 約1.1倍', t312.min > t309a.min && t312.min <= Math.ceil(t309a.min * 1.15), `無=${t309a.min}, 鉢巻=${t312.min}`);
  resetEnv();
  E.sides.self = freshSide('カメックス', 'naminori');
  E.sides.opp = freshSide('カビゴン', 'naminori');
  E.setRandom(() => 0.5);
  E.sides.self.item = '';
  const t312c = E.calcDamage('self', 'opp', moveByName('なみのり'));
  E.sides.self.item = 'muscle_band';
  const t312d = E.calcDamage('self', 'opp', moveByName('なみのり'));
  check('T312b ちからのハチマキ: 特殊には無効', t312d.min === t312c.min, `無=${t312c.min}, 鉢巻=${t312d.min}`);

  // T313 ものしりメガネ: 特殊のみ1.1倍
  resetEnv();
  E.sides.self = freshSide('カメックス', 'naminori');
  E.sides.opp = freshSide('カビゴン', 'naminori');
  E.setRandom(() => 0.5);
  E.sides.self.item = 'wise_glasses';
  const t313 = E.calcDamage('self', 'opp', moveByName('なみのり'));
  check('T313 ものしりメガネ: 特殊 約1.1倍', t313.min > t312c.min && t313.min <= Math.ceil(t312c.min * 1.15), `無=${t312c.min}, 眼鏡=${t313.min}`);
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hataku');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.setRandom(() => 0.5);
  E.sides.self.item = 'wise_glasses';
  const t313b = E.calcDamage('self', 'opp', moveByName('はたく'));
  check('T313b ものしりメガネ: 物理には無効', t313b.min === t309a.min, `無=${t309a.min}, 眼鏡=${t313b.min}`);

  // T314 ちからのハチマキ: マジックルーム中は無効(4件の代表)
  resetEnv();
  E.sides.self = freshSide('カビゴン', 'hataku');
  E.sides.opp = freshSide('フシギバナ', 'hataku');
  E.setRandom(() => 0.5);
  E.sides.self.item = 'muscle_band';
  E.env.magicRoom = 2;
  const t314 = E.calcDamage('self', 'opp', moveByName('はたく'));
  check('T314 ちからのハチマキ: マジックルーム中は等倍', t314.min === t309a.min, `room=${t314.min}, 素=${t309a.min}`);
  E.env.magicRoom = 0;

  resetEnv();
}

console.log('\n=== 段143 レギュMB新ポケモン38体 すべてsimで動作可能か(2026-06-19) ===');
{
  const MB_POKEMON = ['メガライチュウX','メガライチュウY','ラフレシア','ハリーセン','ジュカイン','メガジュカイン','バシャーモ','メガバシャーモ','ラグラージ','メガラグラージ','クチート','メガクチート','メタグロス','メガメタグロス','ムクホーク','メガムクホーク','ムシャーナ','ペンドラー','メガペンドラー','ズルズキン','メガズルズキン','シビルドン','メガシビルドン','カエンジシ','メガカエンジシ','カラマネロ','メガカラマネロ','ガメノデス','メガガメノデス','ドラミドロ','メガドラミドロ','オーロンゲ','タイレーツ','メガタイレーツ','ハリーマン','ハカドッグ','コノヨザル','サーフゴー'];
  let okCnt = 0;
  for (const pname of MB_POKEMON) {
    resetEnv();
    const s = E.makeSideState();
    const poke = pokeByName(pname);
    if (!poke) continue;
    s.poke = poke;
    s.currentHp = E.realStat(s, 'hp');
    if (s.currentHp > 0) okCnt++;
  }
  check('T295 レギュMB38体すべてsimで生成可能', okCnt === MB_POKEMON.length, `${okCnt}/${MB_POKEMON.length}`);

  // T296 メガジュカイン vs リザードン リーフブレード
  resetEnv();
  const lb = Object.values(data.WAZA_MAP).find(m => m.name === 'リーフブレード');
  if (lb) {
    E.sides.self = freshSide('メガジュカイン');
    E.sides.opp = freshSide('リザードン');
    E.sides.self.moves = [lb];
    const r = E.calcDamage('self', 'opp', lb);
    check('T296 メガジュカイン→リザードン リーフブレード', r && r.min > 0, `min=${r && r.min}`);
  }

  // T297 サーフゴー vs フシギバナ 10まんボルト
  resetEnv();
  const elec = Object.values(data.WAZA_MAP).find(m => m.name === '10まんボルト');
  if (elec && pokeByName('サーフゴー') && pokeByName('フシギバナ')) {
    E.sides.self = freshSide('サーフゴー');
    E.sides.opp = freshSide('フシギバナ');
    E.sides.self.moves = [elec];
    const r = E.calcDamage('self', 'opp', elec);
    check('T297 サーフゴー→フシギバナ 10まんボルト', r && r.min > 0, `min=${r && r.min}`);
  }

  // T298 コノヨザル(かくとう/ゴースト) vs カビゴン インファイト
  resetEnv();
  const fight = Object.values(data.WAZA_MAP).find(m => m.name === 'インファイト');
  if (fight && pokeByName('コノヨザル') && pokeByName('カビゴン')) {
    E.sides.self = freshSide('コノヨザル');
    E.sides.opp = freshSide('カビゴン');
    E.sides.self.moves = [fight];
    const r = E.calcDamage('self', 'opp', fight);
    check('T298 コノヨザル→カビゴン インファイト 高威力', r && r.min > 50, `min=${r && r.min}`);
  }
  resetEnv();
}
