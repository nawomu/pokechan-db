 ...
 * ブロック見出し行つき。出力: review/waza_proto_done.html
...
const BLOCKS = [
  { title: '02a 自分が交代(7技)', keys: ['batontatchi', 'tonbogaeri', 'borutochenji', 'kuikkutaan', 'shippokiri', 'samuigyagu', 'sutezerifu'] },
  { title: '02b 相手を交代させる(4技)', keys: ['fukitobashi', 'hoeru', 'tomoenage', 'doragonteeru'] },
  { title: '03 バインド/拘束(適用済 9技)', keys: ['shimetsukeru', 'makitsuku', 'honoonouzu', 'kuroimanazashi', 'uzushio', 'sunajigoku', 'matowaritsuku', 'kagenui', 'torabasami'] },
  { title: '04 2ターン目に攻撃(溜め技 9技/統一適用済)', keys: ['sorawotobu', 'anawohoru', 'daibingu', 'tobihaneru', 'sooraabiimu', 'sooraabureedo', 'erekutorobiimu', 'goddobaado', 'goosutodaibu'] },
  { title: '04r 反動硬直(6技/統一適用済)', keys: ['hakaikousen', 'burasutobaan', 'haidorokanon', 'haadopuranto', 'gigainpakuto', 'gansekihou'] },
  { title: '04s 避けてる相手に命中(5技/状態名で溜め技と対応)', keys: ['naminori', 'kaminari', 'jishin', 'uzushio', 'boufuu'] },
  { title: '05 連続攻撃(15技/4タイプに統一)', keys: ['misairubari', 'boonrasshu', 'tanemashingan', 'tsurarabari', 'rokkuburasuto', 'suiipubinta', 'mizushuriken', 'sukeirushotto', 'daburuatakku', 'tsuinbiimu', 'daburuuingu', "doragon'aroo", 'toripuruakuseru', 'nezumizan', 'fukurodataki'] },
  { title: '06a 与ダメージ反動(9技)', keys: ['sutemitakkuru', 'borutekkaa', 'fureadoraibu', 'bureibubaado', 'uddohanmaa', 'morohanozutsuki', 'wairudoboruto', 'hametsunohikari', 'ueebutakkuru'] },
  { title: '06b 失敗ダメージ(外すと自分も・3技)', keys: ['tobihizageri', 'kakatootoshi', 'sandaadaibu'] },
  { title: '06c HPが減る(コスト・6技)', keys: ['migawari', 'noroi', 'haradaiko', 'sourubiito', 'tetteikousen', 'shippokiri'] },
  { title: '06d 自分がひんし(5技)', keys: ['jibaku', 'okimiyage', 'iyashinonegai', 'inochigake', 'misutobaasuto'] },
  { title: '06e こらえる/接触反動(2技)', keys: ['koraeru', 'niidorugaado'] },
];
...
function row(key) {
  ...
  const effs = scope + ((bd.effects || []).slice().sort((a, b) => rank(a) - rank(b)).map(effLine).join('') || '<span class="dim">追加効果なし</span>') + mll(m);
...
let rows = '';
for (const b of BLOCKS) { rows += `<tr class="sec"><td colspan="14">${esc(b.title)}</td></tr>`; rows += b.keys.map(row).join(''); }
 ...
 *{box-sizing:border-box} body{font-family:-apple-system,"Hiragino Kaku Gothic ProN",sans-serif;margin:0;background:#0f1419;color:#e6edf3;font-size:12px}
 ...
 .c-eff{min-width:330px} .scope{font-size:11px;color:#ffd479;margin-bottom:2px} .el{display:flex;align-items:baseline;gap:5px;padding:1px 0;line-height:1.5;flex-wrap:wrap}
...
<header><h1>やった分 — 交代系 + バインド/拘束(${BLOCKS.reduce((a, b) => a + b.keys.length, 0)}技・プロト形式)</h1>
...
<table><thead><tr>
 <th>習得</th><th>わざ名</th><th>①優先</th><th>フラグ</th><th>タイプ</th><th>分</th><th>威力</th><th>命中</th><th>PP</th><th>接触</th><th>守貫</th><th>効果(②威力→命中後→攻撃後→場/継続→終了)</th><th>タグ</th><th>説明(ポケモン徹底攻略)</th>
</tr></thead><tbody>${rows}</tbody></table>
</body></html>`;
fs.writeFileSync(path.join(ROOT, 'review/waza_proto_done.html'), html);
console.log('生成: review/waza_proto_done.html /', BLOCKS.reduce((a, b) => a + b.keys.length, 0), '技');
