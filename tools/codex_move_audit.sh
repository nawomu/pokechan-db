...
最後のメッセージは次のJSONだけを出力(前後に文章を付けない): {\"name\":str,\"slug\":str,\"checks\":[{\"aspect\":str,\"ours\":str,\"authority_quote\":str,\"verdict\":\"match|mismatch|missing_in_ours|extra_in_ours|unknown\",\"note\":str}],\"summary\":str}"
  codex exec -C "$ROOT" --sandbox read-only --output-last-message "$OUT/.tmp_found.json" "$P1" >/dev/null 2>"$OUT/.tmp_err1.log" || { echo "FAIL(照合) $slug"; cat "$OUT/.tmp_err1.log" | tail -2; break; }
  FOUND=$(cat "$OUT/.tmp_found.json")
...
最後のメッセージは次のJSONだけ: {\"name\":str,\"slug\":str,\"agreed\":bool,\"overturned\":[{\"aspect\":str,\"why\":str,\"authority_quote\":str}],\"missed\":[...同型],\"summary\":str}"
  codex exec -C "$ROOT" --sandbox read-only --output-last-message "$OUT/.tmp_reb.json" "$P2" >/dev/null 2>"$OUT/.tmp_err2.log" || { echo "FAIL(反証) $slug"; break; }
  node -e '
...
' "$OUT" "$OUT/.tmp_found.json" "$OUT/.tmp_reb.json" "$slug" || { echo "FAIL(JSON) $slug"; break; }
  n=$((n+1))
done
