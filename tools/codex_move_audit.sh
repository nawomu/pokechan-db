...
MODEL_ARG=""
[ -n "${CODEX_MODEL:-}" ] && MODEL_ARG="-m ${CODEX_MODEL}"   # 例: CODEX_MODEL=gpt-5.3-codex-spark(軽量・別枠)
...
最後のメッセージは次のJSONだけを出力(前後に文章を付けない): {\"name\":str,\"slug\":str,\"checks\":[{\"aspect\":str,\"ours\":str,\"authority_quote\":str,\"verdict\":\"match|mismatch|missing_in_ours|extra_in_ours|unknown\",\"note\":str}],\"summary\":str}"
  codex exec $MODEL_ARG -C "$ROOT" --sandbox read-only --output-last-message "$OUT/.tmp_found_${BATCH}_$SHARD.json" "$P1" </dev/null >/dev/null 2>"$OUT/.tmp_err1_${BATCH}_$SHARD.log" || { echo "FAIL(照合) $slug"; cat "$OUT/.tmp_err1_${BATCH}_$SHARD.log" | tail -2; break; }
  FOUND=$(cat "$OUT/.tmp_found_${BATCH}_$SHARD.json")
...
最後のメッセージは次のJSONだけ: {\"name\":str,\"slug\":str,\"agreed\":bool,\"overturned\":[{\"aspect\":str,\"why\":str,\"authority_quote\":str}],\"missed\":[...同型],\"summary\":str}"
  codex exec $MODEL_ARG -C "$ROOT" --sandbox read-only --output-last-message "$OUT/.tmp_reb_${BATCH}_$SHARD.json" "$P2" </dev/null >/dev/null 2>"$OUT/.tmp_err2_${BATCH}_$SHARD.log" || { echo "FAIL(反証) $slug"; break; }
  node -e '
...
const F=j(f),R=j(r);
if(F.slug!==slug||R.slug!==slug){console.log("REJECT(slug不一致="+F.slug+"/"+R.slug+")",slug);process.exit(3);}
fs.writeFileSync(o+"/"+slug+".json",JSON.stringify({slug,found:F,rebuttal:R,source:"codex"},null,1));console.log("OK",slug);
' "$OUT" "$OUT/.tmp_found_${BATCH}_$SHARD.json" "$OUT/.tmp_reb_${BATCH}_$SHARD.json" "$slug" || { echo "FAIL(JSON) $slug"; break; }
  n=$((n+1))
done
