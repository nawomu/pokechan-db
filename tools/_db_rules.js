...
// ルール(R1〜R8)
const RULES = [
  ['R1', '共通SSOT 1本化(最重要)= データは一つ。全ページ(バトルも)大元だけを読む',
    '<b>大元(マスターデータ)= <code>master/*.json</code> ただ1つ</b>(修正・追加はさらに上流の <code>reference/_*_additions.json</code> / <code>_*_fixes.json</code> / <code>_regulations.json</code>)。<code>tools/build_master_v2.js</code> → <code>master/</code> → <code>tools/build_views.js</code> → ページ用生成物(<code>pokechan_data_all.js</code>/<code>pokechan_data.js</code>/<code>items_database.js</code>)。ページは <code>pokedb.js</code> 1枚から読む。<br>'+
    '<b>★全ページ共通(2026-09-03 阿部さん再確認)</b>: 全国版・Champions版・<b>バトル(sim/リアルバトル)</b>・図鑑・わざリスト・持ち物・多言語…<b>どのページもデータは大元だけ</b>。<b>ページ上でデータを勝手に作らない</b>(ページ専用JSON/JS・コピー・文字列直書きは全部禁止)。足りなければ<b>大元を広げる</b>。理由=<b>一箇所変えれば全部変わる</b>ようにするため(ページが増えるほど参照がずれる→全部直す→全部チェックし直す、を止める)。<br>'+
    '<b>更新運用</b>: 新ポケ/技/持ち物/レギュ → ①<code>reference/_*_additions.json</code>/<code>_regulations.json</code> に足す → ②<code>build_master_v2.js</code> → ③<code>build_views.js</code> → ④番人(<code>_views_diff</code>/<code>_ssot_guard_test</code>/実機) → <b>HTMLは無変更</b>。これが「追加はデータに足すだけ」の達成形。'],
  ['R2', '既存デザインのクローン方式',
  ...
  ['R4', 'Generation と Season(レギュ)は別項目。Season は「現行」と「次」の2つだけ持つ',
    '<b>Generation(Gen1〜9)</b>=dex由来・全ポケにあり・表記は翻訳不要(Gen1)。ポケモンDBの「Gen」列で表示。技の世代は R10。<br>'+
    '<b>Season(レギュレーション)</b>=Champions のシーズン。<b>★持つのは「現行」と「次」の2つだけ(2026-09-03 阿部さん決定)</b>。例: いま=M-B(本番で遊べる)/ 次=M-C(発表済み・9/9から)。M-Cが本番になったら M-C=現行・M-D(発表されたら)=次、と<b>2枠を回す</b>。M-Aのような終わったレギュは行の項目から外す(年に何回も増えて溜まる一方になるのを防ぐ。過去の名簿は <code>reference/_official_rosters/&lt;REG&gt;.json</code> と git履歴に残る=消えはしない)。「前のレギュに戻る」は想定しない。<br>'+
    '<b>なぜ2つか</b>: 本番の更新より<b>先に</b>次のレギュで遊べる/情報が見えるようにするため。「M-Bはまだこれ・M-Cはこれ」が同時に分かる必要がある。次のレギュが発表されたら、<b>公式開始を待たず</b>そのレギュのポケモン/技/持ち物をどんどん「次」に足してよい(レギュは累積=M-Bで使えたものはM-Cでも使える。抜けるポケモンが出たら「次」から外す)。<br>'+
    '<b>データの持ち方</b>: 行ごとに「使えるレギュ」= 現行/次のどちら(or両方)か(<code>seasons</code>/<code>regulation</code>)。<code>master/regulations.json</code> に現行と次の2件(どちらが本番かの印つき)。ページ側は<b>現行を既定表示・次に切替できる</b>(SSN列/リアルバトルの選出)。全国版テーブルには出さない(非Championsが大半=空列回避)。'],
  ['R5', '伝説区分マーク',
