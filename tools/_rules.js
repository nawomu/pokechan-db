...
const WRITE = [
  ...
  ['★未実装システム(ダイマックス等)は「(カッコ書き)」で残す', '<b>★2026-06-17 阿部さん変更(2026-06-15「非表示」を上書き)</b>: 未実装システム(ダイマックス・ダイウォール・Zワザ・テラスタル)の言及は、<b>非表示にせず文末に（…）のカッコ書きで残す</b>。例: けたぐり「…)。（ダイマックスしている相手には効かない）」／なみだめ「…の効果を受けない（ダイウォールは除く）」／まもる「…（ダイマックス技やZワザの攻撃は防いでも最大HPの1/4のダメージを受ける）」。対象=ダイマックス系すべて。実装=<code>SYSTEMS_IN_GAME</code> が false の系統を <code>gatedItems()</code> でカッコ書き、来たら <code>true</code> でカッコ無しの通常文に一斉切替(書き直し0)。判定者の「ダイマックス無効がデータに無い」は<b>従来どおり誤検知</b>。<br><b>(以下・旧2026-06-15方針=今は無効・記録のみ)</b> <b>ポケモンチャンピオンズに現状無いシステム(ダイマックス等)への言及は、説明文に出さない</b>(2026-06-15 阿部さん)。理由=<b>北極星はこのゲームの子ども</b>・SSOTはこのゲーム。存在しない仕組みの説明は子を混乱させる(「ダイマックスって何?」=意味が濁る)。legacyにある「ダイマックスしている相手には無効」等は<b>今は出さない</b>。<br>ただし<b>将来来た時の作業を最小化</b>するため、表示は compose のフラグ(<code>DYNAMAX_IN_GAME=false</code>)で<b>ゲート</b>し、来たらフラグを <code>true</code> にするだけで全技に一斉表示できるようにする(書き直しゼロ)。データ(effects)への一律付与は「重さ依存技・強制交代技はダイマックス相手に無効」のような<b>一律ルール</b>なので、来た時に <code>_fix</code> 一本で機械的に付けられる→<b>今あわてて入れない</b>。<br>さらに<b>将来のチャンピオンズのダイマックスが本家と違うルール(季節制等)で来る可能性</b>があり、文言を今固めると間違いになって書き直しになる恐れ→<b>文言は今固めず据え置く</b>。※独立検証(sonnet)が「ダイマックス無効がデータに無い」をデータ不足に挙げるのは<b>従来どおり誤検知</b>。'],
  ['★effectsはsim前提=フェーズを意識', 'effects は<b>バトルシミュレーター(real_battle_simulator.html・Phase構造v7)が読み取る前提</b>で作られたデータ。説明文(compose)は<b>「simが読むeffectsをただ日本語に訳しているだけ」</b>。だから effects は<b>常にsimとセット</b>で、<b>simの流れ＝フェーズ(優先度→攻撃前の変化→攻撃→攻撃後[適用後]→ターン終了)を意識して構築</b>する。composeが"賢く"織り込み・並べ替えをしたくなったら、それは<b>effectsのphase設計が足りないサイン</b>→エンジンに特例を足さず<b>データ(phase)を整える</b>。実機バトルの流れ確定リファレンス=<b>バトルの流れ_実機リファレンス.md</b>(出典つき実機調査＋sim「Phase構造v7」突き合わせ)。(2026-06-07 阿部さん)'],
