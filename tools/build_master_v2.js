...
function buildItems() {
  ...
  // ★2026-09-10 レギュM-C本番反映: PokeAPI暫定で足した行(エレキシード等)がChampionsのショップに来た時、上の fixes 適用は
  //   その行がまだ無い時点で走っている(順序)。器を広げた後にもう一度 fixes を当てる(同じ規則・同じ入口=二重管理しない)。
  ...
  try {
    ...
    items.forEach(it => {
      ...
      if (Array.isArray(it.provisional_fields) && it.provisional_fields.includes('all')) {
        ...
        it.provisional_fields = ['pokeapi_id', 'pokeapi_slug', 'names', 'cost', 'fling_power', 'gen_introduced', 'category_pokeapi', 'pocket', 'effect_en', 'flavor_ja', 'flavor_en'].filter(k => !fixed.has(k));
      }
    });
  } catch (e) {}
