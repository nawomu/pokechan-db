...
// ★2026-09-04(B-4是正): タイプ相性表の直書きを master/types.json の meta.tables.TYPE_CHART 参照に統一
//   (2026-09-03 の a380b67b で3本を統一した際、この4本目を見落としていた=監査 review/_page_data_audit_r2 指摘)。
//   master/types.json の items 配列の並び順が TYPES と一致していることを起動時に検証してから使う(不一致なら止める)。
const TYPES_MASTER = require(path.join(ROOT, 'master', 'types.json'));
const _typesOrderOk = TYPES_MASTER.items.map(t => t.name).join(',') === TYPES.join(',');
if (!_typesOrderOk) { throw new Error('TYPE_CHART: master/types.json の items 順序が TYPES 配列と一致しません(index対応が崩れる)。'); }
const TYPE_CHART = TYPES_MASTER.meta.tables.TYPE_CHART;
