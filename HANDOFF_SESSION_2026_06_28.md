...
- **★残=needs_compose 約70件**(`reference/_needs_compose.json`)= effectsは在るが**composeが喋れていない/出し分け不足**: ①コピー系の出し分け(ものまね=一時習得/オウムがえし=即時再現/スケッチ=永久習得 が同一文)②ランダム技の母集団(ゆびをふる=ほぼ全技/ねこのて=手持ち仲間)③条件・発動条件描画(ゆめくい=ねむり限定/おいうち=交代時2倍/しおみず=HP半分2倍/かたきうち=味方倒れ2倍)④期間(どろあそび/みずあそび=5ターン)⑤連携(クロスフレイム/サンダー=ペア2倍)⑥Zワザ注記 等。**次工程=これらのrenderをcomposeに足す**(effectsは構築済なので主にcompose側)。
