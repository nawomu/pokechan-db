...
## 7. ★ふりかえり(なぜ失敗していたか / どう直すべきか / 本来こうやるべきだった)

### A. 何が原因で壊れていたか(過去〜今回の根本)
1. **ヤックン(お手本)が番号ズレで全面崩壊**: 前のセッションが`moves_yakkun.json`を「Yakkun技番号=PokeAPI move id」と誤って番号で突き合わせて作った。実際は両者がズレる(ダイサンダー=Yakkun741/PokeAPI759)ため、**別技の説明が大量に紛れ込んだ**。お手本が壊れていたので、そこから作った効果文も全部おかしかった。
2. **パイプラインを逆走していた**: 新技445は「英語から日本語を直接手書き(moves_ja_desc.json)」で、**effects→compose を通っていなかった**。だから effects と説明文が無関係に乖離し、effectsは薄い/空のまま。出発点(effects先・compose訳すだけ)から外れていた。
3. **私自身も漂流した**: 121kindに無いメカ(コートチェンジ等)に出会ったとき、**effectsを空のまま説明文だけ手書き(override)して「埋まった」と思った**。阿部さんに「effects空=simが動かない・出発点に反する」と指摘されるまで気づかなかった。=「作る実感>確かめる規律」の漂流([[verification-over-production]])。
4. **"穴0=完成"と誤認**: effectsを構築して`compose穴0`になると「できた」と思ったが、**穴0は「composeがエラーを出さない」だけで、出力がヤックンの意味と合っているかは別**。実際 照合WTで427中163がズレていた(voiced≠complete)。
5. **系統的欠落を後で発見**: 技フラグ(音/風/切る)未適用・優先度未設定・範囲「相手全体」未描画 等が、**全部作り終えてから**照合で大量に露呈した(163件の多くがこれ)。
6. **Workflowの初歩ミスで空振り**: スクリプトに`require`を書く/`args`が文字列で来る、を毎回踏んで再実行。セッション上限にも複数回到達。

### B. どう直したか(今回の改善)
- お手本を**英語名で突き合わせ**て取り直し(番号に依存しない)。
- effectsを正データに据え、**説明文をcompose一本化**。語彙外は**手書きでなく新kindを足す**。
- 照合を**workflowで意味照合**し、**系統的欠落を一括補修**(163→77→約405/427 ok)。`compose穴0`に加え**undefined0・実行エラー0**も毎回ゲートに。

### C. ★本来こうやるべきだった(次に同種をやるときの正解手順)
1. **まずお手本(ヤックン)を名前で正しく取り込む**。番号でなく名前で join。壊れた参照の上に何も建てない。
2. **小さく端まで通す(20技パイロット)**: 20技で `effects→compose→**ヤックンと意味照合(手で)**` を端まで実行。ここで **systemicな欠落(フラグ/優先度/範囲/未対応kind/未対応条件render)を全部洗い出して compose を先に直す**。
3. **その後に量産**。量産後の検証は「**compose出力↔ヤックンの意味**」(穴0だけで満足しない)。
4. **effects空で説明文だけ手書きは絶対しない**。語彙が無ければ新kindをスキーマ＋compose(＋sim)に足す。
5. **Workflowは骨組みを1回テスト**してから本実行(require禁止・args文字列ガード)。大量実行は**ペース配分**して上限回避。

→ 一言で: **「お手本を正しく・小さく端まで通して系統穴を先に潰す・穴0でなく意味で検証・逆走(手書き)しない」**。今回はこれを後追いで学んだので 163→77→… と何往復もした。次は最初からこの順でやれば一発に近づく。
