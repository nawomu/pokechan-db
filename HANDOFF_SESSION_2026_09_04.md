...
- W20(新版ページ群の壊す側レビュー)→ §6。是正 W21 `264ac507`・特性名 W22 `d7f1c979` → §6b。③着手前の読み取り専用準備(W23〜W27)→ §7-4: 棚卸し `dca3a9a9` → 87件再判定 `dc6f943b` → 新規11草案 `2a6dce5a` → 重なり15照合 `e438eb6c` → **③バックログ票1本 `df800b8f`/`fdbdb986`**(`review/_battle_rework_backlog_2026-09-05.md`・168件・阿部さん判断は §5 の 2/3/7)。**夜間ぶんの push は全部 main に載っている(最終 = `fdbdb986`・9/5 02:35)。06:00 に M-C 一覧はまだ未掲載のままループ終了。**
...
1. ~~**旧版の引退**: speed_compare / suggest_lite / suggest_partner / damage_calc / party_checker / pokemon_db_v9 / pokemon_db_all_v9 / waza-list / waza-list_all(+`waza_picker.js`)。新版で置き換えてよければ nav/canonical を新版へ向けて旧を削除(順序: 新版を正典URLに→旧削除)。~~ → **全部解決(2026-09-05 昼 阿部さん「新版で確定・旧版はいらない・補完ファインダーは詳細サジェストへ統合」・`dab55e38`)**: speed_compare/suggest_partner/damage_calc/party_checker=同名改名、`pokemon_db_v10`→**`pokemon_db.html`**(正典名新設)、旧URL `pokemon_db_v9`/`pokemon_db_all_v9`/`suggest_lite` は**転送スタブ**(データを読まない・noindex)、suggest_lite は旧新とも削除。`pokemon_db_all_v10.html` は**名前据え置き**(`pokemon_db_all.html` を更に古いページ[2026-06-26 全1302・_old_master生成]が占有しているため)→ **解決(同日昼 阿部さん「新しいのに差し替えて、旧版はいらない。全国版ね」)**: 旧々版 `pokemon_db_all.html`+凍結生成器 `tools/_build_pokemon_db_all.js` を削除、all_v10 を **`pokemon_db_all.html`** に改名(旧名 _v10/_v9 は転送スタブ)。同日: speed_compare の見出し黒帯ずれ修正(旧版由来)・suggest 2枚でメガを候補に含める(`53826854`)。sitemap.xml は生成器 `build_i18n_pages.js buildSitemap()` が実物(232k行)から乖離しているため手で2行だけ直した(生成器にコメント済み・再生成禁止)。
