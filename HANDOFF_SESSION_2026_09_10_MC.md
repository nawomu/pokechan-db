...
| B064 D5-1 | **完了・検収済み** `9d70f081b`(+`01255dd46` 枠1の currentHp 初期化 / `deb60dbb5` __sim 収録): 開発プレビュー **`battle_doubles_preview.html`**(noindex・導線非公開・pokedb.js のみ・9言語キー20・対象選択・枠交代・メガ・死に出し・勝敗・既存ログ文言のみ)。実機10回連続合格・i18n 8言語 残日本語0。★**ページ台帳に暫定登録(阿部さん未承認)**=下の🙋。自分側の選出は自動抽選(手動選出UIは未)。観測=かたやぶり登場ログの特性名が未翻訳 → `f8494d1ea` で修正済み(battle_log_i18n の when 述語) | 上記 |
...
| D6-5 | オンラインのロックステップに枠と対象(`{fmt:'double', slots:[…]}`・対象の側は鏡写し・枠ごとの pivotIdx 待ち行列)・枠ごとの死に出し(両側同時・canonSlots 順)・**ダブルのオンライン解禁**・**ループバック検証** `tools/_online_doubles_loopback_playwright.js`(Supabase なし・BroadcastChannel スタブ・12ターン鏡写し不一致0)・D6-3 レビュー修正(高-1 対象板の COMMAND 札/中-1 同種の同時選出禁止/中-2 交代枠の技を消す 他)。★実サーバ2ブラウザは Supabase 復旧後=阿部さん | `daa556547` |
| 入口 | トップページに「ダブルバトル」カード(`online_battle.html?format=double`)/ 開発プレビュー `battle_doubles_preview.html` 削除(台帳の約束どおり) | `06caf4954` `fb2933779` |
| 監査 | ワークフロー(20機構・Wiki 引用→fixture→実行→反証)= `tools/_doubles_audit/`(`2c1fb359c`)。pass1/fail4/未実装12/blocked2 + 土台の穴(枠0限定ループ) | |
| E1 | エンジン: 枠0限定処理17箇所を全枠化・側の欄(おいかぜ/壁/設置物)・範囲技の二重ロール修正・しめりけ全枠・ミラーアーマー-6不発・いやしのすず味方枠。fixture 67/0・監査5本緑化 | `16a1c6d0e` |
| E2 | 味方を読む特性10機構(土台 allySlotsOf・フレンドガード/テレパシー/プラマイ/そうだいしょう/おもてなし/きみょうなくすり/いやしのこころ 50%/レシーバー/きょうせい/ベール3)・ものまねハーブの実バグ修正。fixture 84/0・監査 48/51 | `d449ae3d1` |
| データ | いやしのこころ effect_en 50%(fixes・builder が effect_en を PokeAPI 合流後に適用・i18n en は master を正) | `6f794e97b` |
| E3 | ワイドガード/ファストガード(側のまもり・WG はシングルでも範囲技を防ぐ)・サイドチェンジ(位置入替)・ロックステップ正準化(forEachSlotItemOrder)。fixture 92/0・監査 WG/FG 7/7・AS 4/4 | `94fa57e0a` |
| D6-5 修正 | 壊す側レビュー: 時間切れで対象板が固まる/死に出し2ラウンド目の上書き desync/同種の同時選出=図鑑No.基準(Wiki+公式 竜王戦+Champions サポート)/pivot 枠つき/turnNo・COMMAND 一本化 他。ループバック17ターン不一致0 | `373093956` |
| E4 | 死に出しの登場効果=全員出揃ってから(deferEntry/flushDeferredEntries・補正抜き素早さ実数値・TR不問=Wiki おもてなし)・実機メッセージ4行 9言語(bl_261〜264)。監査 E2 51/51・fixture 96/0 | `eaf782874` |
| E5 | **実装中**: E1〜E3 壊す側レビューの修正(サイドチェンジ後に味方の行動が消える/ターン終了順=効果すばやさ+TR反転を道具反応(素の実数値)と分ける/wish 等は位置に残る/ファストガードが特性由来の優先度を防ぐ/だっしゅつボタン最速1体/mist の snapshot 他)指示書 scratchpad `spec_e5_review_fixes.md` | — |
| ページ仕上げ | **実装中**: モバイル横向き CSS・枠1の pivot 事前選択・おまかせ編成の図鑑No.基準・E4 配線(deferEntry/flush・?v) | — |
| 台帳送り(E3/E5 レビュー) | WG/FG「ターンの最後に使うと失敗」(監査 fixture C5 と両立しない=阿部さん判断)/ ダブルの undo 枠1 / ターン終了ブロックの内訳順(グラス→たべのこしを1体ずつ・どく/やけどより前=シングルも動く別票)/ ねらいうち・スクリューおびれ・すじがねいり(サイドチェンジ無視)/ 「もちもの重複禁止」の選出時関所の有無 | |
- 各段=実装(Opus)→壊す側レビュー(Opus・読み取り)→ゲート(シングル不変・PDCA・i18n・sim diff 0)→commit/push。検証スクリプト= scratchpad `verify_d6_*.js`。
- 本番で試せる: `https://pchamdb.com/online_battle.html?format=double`(トップの「ダブルバトル」カード)→ AIと たいせん / オンライン対戦(D6-5 で解禁・実サーバ検証は未)。
